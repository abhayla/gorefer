...
## Deploy headline
GoRefer is **LIVE in production AND live-integrated** at **https://gorefer.in** — Hostinger VPS `72.61.240.224`, Cloudflare proxied, SSL Full-strict, live since 2026-07-09; deployed SHA `2eff029`. **The go-live flips happened ~2026-07-17/18**: `ENABLE_WATI_SEND`, `ENABLE_ZOHO_WRITE`, `ENABLE_ZOHO_READ` are **ON** (Settings/cascade overrides — the `.env` defaults still read `false`; never judge flag state from `.env`), the WATI recipient allowlist is **open** (real sends daily), and the **Zoho conversion webhook is live with the DF-2 HMAC seal ON** (signer + workflow rule active in Zoho; conversions ingesting — first live ingest `RJ4521`, 18-Jul). **M13 referrer login is LIVE (2026-07-21, PR #20)**: `ENABLE_CUSTOMER_LOGIN` + `ENABLE_OTP_LOGIN` are **ON** — `/login/` (Google OAuth primary + WhatsApp-OTP fallback), Path-B evidence verification, `/my/referrals` self view, admin Verifications queue. OTP AUTH template `gr_platform_gorefer_login_otp_en_2026_07_21` APPROVED + delivery-verified. Daily ops instrument: the three-sided 21:30 IST delivery+funnel report (O-6a/R-DRR, `Wati-Project/daily_report.py`).
...
| **M13 — Customer/referrer login + Client-ID binding + self "My Referrals"** | **Deployed (LIVE 2026-07-21)** | Flip `ENABLE_CUSTOMER_LOGIN`, OAuth, Zoho-verified identity binding, role-scoped self-view. **This is the gate that unblocks Q-M-OTP merge.** Spec `S2-01`/`S2-03`, ADR-035. |
...
2. **The Sprint-2 customer-login gate (M13) is OPEN and LIVE** (2026-07-21, PR #20): OAuth + OTP doors, identity binding (auto-bind + admin queue + Path-B evidence), `/my/referrals`; contract `docs/sprint2/S2-05`. Remaining follow-ups: DF-OTP-SMS (optional SMS fallback) and Hindi parity for login surfaces if wanted.
