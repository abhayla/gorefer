...
> **Last updated:** 2026-08-04. Owner: Abhay/PIFS.
...
| DF-11 | Self-click tagging on the Referral Profile | 🔵 Partial (display half done, T-060 2026-08-09) | P3 | **Display tag SHIPPED** — Clicks tab tags a click "self-click" when the click's promoted lead-side mobile (ADR-018) matches the referrer's own Zoho-read mobile, resolved-flag-gated, DISPLAY-ONLY. **Count-exclusion half still open**, same trigger: self-referral inflation concern / after customer view ships. |
...
| Q-M-OTP-2 | Zoho `client_id → on-file channel` READ wiring (for login OTP Path A) | ✅ Done (2026-07-21, M13 / PR #20) | P2 | **CLOSED** — `recipient._from_zoho` wired to the M9 READ adapter (`ClientId → Mobile/Phone`, normalized, resolved-flag-gated, degrades to Path B on outage). READ field-set extended with Mobile/Phone/Email (contract-doc'd). Live in prod with `ENABLE_OTP_LOGIN=true`. |
...
| DF-PORTS-1 | Evict report-composition halves of `wati/engagement.py` + `wati/notify.py` from the boundary | 🟡 Open | P3 | Deliberately kept inside the boundary by ADR-048 (doc 17) — a second messaging vendor, or the Wave-4 shared gateway |

**Open go-live preconditions (not build items, but tracked):** (1) Fix Wati's ~60% delivery reliability — **queue live, measuring toward ≥90% (DF-WATI-REL)**; (2) ~~Meta-approve the Wati templates~~ **DONE** — the login-OTP template is now `gr_platform_gorefer_login_otp_en_2026_07_21` (AUTHENTICATION, **APPROVED 2026-07-21** + delivery-verified; the old `gorefer_login_otp` name was never submitted); (3) set `WATI_WEBHOOK_KEY` on prod to run the WhatsApp assisted-capture E2E. `ENABLE_WATI_SEND` / `ENABLE_ZOHO_READ` flipped ON ~17-Jul; the login flags flipped ON 21-Jul (M13).
...
- **Status (T-060, 2026-08-09): DISPLAY half shipped.** `apps/dashboard/profile.py:clicks_rows` tags a click `self_click=True` when the click's window contains a `lead_captured` event whose Prospect mobile (ADR-018 promotion) normalizes to the same value as the referrer's own Zoho-read mobile/phone (`_referrer_own_mobile`, resolved via the same `get_crm_read_port()` the rest of the profile already uses — degrades to no-tag on an unmatched contact or an unreachable Zoho, never a guess). Purely a rendered badge (`static/js/referral_profile.js`) — **zero analytics/rollup/count mutation**; every aggregate (`top_band`, `per_link_cards`, rollups) is untouched.
- **Still open — the exclusion half:** actually excluding a self-click from conversion/click counts is NOT built (would touch `top_band`/`per_link_cards`/rollup aggregates, which is a bigger, riskier change than a display tag).
...
### DF-PORTS-1 — Evict report-composition halves of `wati/engagement.py` + `wati/notify.py` from the boundary
- **What:** `apps/integrations/wati/engagement.py` (WA engagement report) and the transport half of `apps/integrations/wati/notify.py` currently mix vendor-API reads with report/message composition logic, both living inside the `apps/integrations` boundary. This item is to split out the composition halves (the parts that don't read a vendor API) into the relevant domain app(s), reached only via the `apps/integrations/services.py` facade.
- **Why deferred (doc 17, ADR-048, 2026-08-04):** the doc-17 vendor-port-boundary hardening program (ADR-045…048) deliberately left these two modules whole inside the boundary — swappability is defined as "domain never imports vendor directly," which Waves 1–3 already achieve. Splitting composition out of `engagement.py`/`notify.py` is cosmetic eviction with no swappability benefit today, so it was scoped out to avoid unnecessary churn (doc 17 §"Wave contracts", scope discipline note).
- **Revisit when:** a second messaging vendor is integrated (composition/transport split becomes load-bearing for a real swap), or the Wave-4 shared gateway service (doc 17's parked cross-project reuse wave) is built and needs a clean composition layer to call into. Relates to ADR-045, ADR-046, ADR-048, `docs/architecture/17-Vendor-Port-Boundary-Hardening.md`.
