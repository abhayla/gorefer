...
The webhook + `/r/` redirect must be **publicly reachable at gorefer.in** for Wati to call the webhook and for prospects to click. So a deploy of at least the redirect + webhook is required before the end-to-end live test. **Deploy target is LOCKED: the Hostinger VPS `72.61.240.224` (Linux nginx + certbot), NOT the local box `103.118.16.189` — see `docs/deploy/DEPLOY-TARGET.md` (authoritative).**
