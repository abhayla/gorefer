...
- **Status:** Locked (2026-07-06, added after the external-review walk; authoritative decision log `gorefer-ops/review/Review-Matrix-v1.md`, new deferrals `gorefer-ops/review/Deferred-Features-Backlog.md`).
...
Reliability (not edge): automated backups + tested restore + health checks. **Corrected 2026-08-17 (T-162, owner-approved point 24A):** this line previously read "managed DB + automated backups + standby + health check". There is **no standby** and the DB is **not managed** — GoRefer runs a self-hosted Postgres on the single Hostinger VPS `<PROD-VPS>`. The real posture is: **nightly `pg_dump`** (02:30 IST, `/usr/local/bin/gorefer-pg-backup.sh`, 14-day rotation, archive verified with `pg_restore --list`), a **restore that has actually been tested** (2026-07-20, 41 tables into a scratch DB), an **off-VPS copy of the newest dump** (`scripts/backup_offsite.sh`, T-162), and health checks (`/api/health`, `/api/health/worker`). That gives **RPO ≈ 24 hours** (worst case = everything since last night's dump) and **RTO = provision a box + restore the dump + repoint DNS**, i.e. **hours, by hand, with no automated failover**. Saying "standby" implied seconds of downtime and zero data loss; the truth is a day of data and a manual rebuild. Tightening either number is an ops decision (a second box, streaming replication, or more frequent dumps), not an architecture change — ADR-021's central-runtime decision stands either way.
...
- **Status:** Locked (2026-07-06). Chosen after a neutral external cross-check (prompt: `gorefer-ops/review/Framework-Evaluation-Prompt.md`) run past **ChatGPT, Grok, Gemini, DeepSeek** plus a supplementary framework analysis. Full basis + trade-offs: **`gorefer-ops/review/Framework-Decision-Synthesis.md`** and the per-LLM captures `gorefer-ops/review/{ChatGPT,Grok,Gemini,DeepSeek}-Framework.md`. Encoded into the build guide `implementation/10` §1.
...
- **Identity model decision:** **no link-claiming step** — a login only *unlocks the retroactive view* of everything already keyed to the Client ID (lazy-journey creation recorded it from the first click). Ownership is proven, not asserted:
  - **Path A (known referrer, Client ID in Zoho):** resolve the Client ID → the **on-file** mobile/email in Zoho (verified live 2026-07-11: Zoho Contact carries `ClientId` + `Mobile`, e.g. QPJ023 → 9999900004) → send an **OTP to that on-file channel** (never a typed number) → bind the login to **`(tenant_id, client_id)`** (ADR-023).
