...
> **This header + the Deployed-SHA row had drifted THREE deploys behind prod**, discovered only
> by SSH-reading `DEPLOYED_SHA` directly: prod was already at `5d70adc` (T-097 advisor-callback
> capture, PR #156, merged 2026-08-11) before this session started, and further merges landed
> `dba64ea` (T-104 slot-tap callback, PR #158) + `7141c5a` (T-098 concurrency fix, PR #159) — all
> three deployed to prod without a matching COORDINATION/CURRENT-STATE update. Verified by
> direct SSH read of `/var/www/gorefer/DEPLOYED_SHA`, not by trusting this file. This deploy
> (`scripts/deploy.sh` run) carries all of T-097/T-104/T-098 forward plus the deploy-runner
> commit itself, hash-verified byte-exact, `migrate`/`collectstatic`/`check` clean (2
> pre-existing SSL warnings — TLS terminates at Cloudflare per DEPLOY-TARGET.md), all 3 services
> (`gorefer`, `gorefer-qcluster`, `nginx`) active, `DEPLOYED_SHA` written, `/api/health` probed
> **200**. T-061 Hindi parity (merge `a94560e`) confirmed an ancestor of the deployed commit —
> live in prod continuously since 2026-08-09, unaffected by this deploy. Homepage 200 with the
> NSE AP disclosure block + market-risk warning intact. See `docs/deploy/DEPLOY-TARGET.md`
> "Deploy runner" section for usage.)
> — prior 2026-08-10 20:00 IST (**DEPLOYED: `d238789` = main tip** — **T-081 OTP email
